...
f_costs_df = pd.read_csv("FixedCosts.csv")
w_regions_df = pd.read_csv("WarehouseRegions.csv")
o_regions_df = pd.read_csv("OrderRegions.csv")

warehouses = list(range(1, costs_df.shape[0] + 1))
orders = list(range(1,costs_df.shape[1]))
products = list(range(1,weights_df.shape[0] + 1))
regions = []  # filled on line 78
...
'''
f = {}
for index,row in f_costs_df.iterrows():
    for o in orders:
        f[int(row[0]),int(o)] =  float(row[o])

'''
Creates rw:
Dictionary mapping region to the warehouses belonging to that region.
'''
rw = {}
for index,row in w_regions_df.iterrows():
    if int(row['Region']) not in rw.keys():
        regions.append(int(row['Region']))
        rw[int(row['Region'])] = [int(row['Warehouse ID'])]
    else:
        rw[int(row['Region'])].append(int(row['Warehouse ID']))
regions.sort()

'''
Creates ro:
Dictionary mapping region to the orders belonging to that region.
'''
ro = {}
for index,row in o_regions_df.iterrows():
    if int(row['Region']) not in ro.keys():
        ro[int(row['Region'])] = [int(row['Order ID'])]
    else:
        ro[int(row['Region'])].append(int(row['Order ID']))
...
'''
Regional Constraint:
Each order must be fulfilled by the warehouses in its region first, so we use dictionaries 
rw and ro to constrain the demand and supply to single regions in order to consider them
first in deciding from where to source the products.
'''
for r in regions:
    m.addConstrs(quicksum(x[i,j,k] for i in rw[r] for j in ro[r] if (j,k) in d.keys() and (i,k) in s.keys()) == min(
            sum(d[j,k] for j in ro[r] if (j,k) in d.keys()),
            sum(s[i,k] for i in rw[r] if (i,k) in s.keys()))
            for k in products)
...
for j in orders:
    for i in warehouses:
        for k in products:
            if x[i,j,k].x > 0:
                print('Order %i was sent %f units of of Product %i from Warehouse %i' % ( j, round(x[i,j,k].x), k, i))

print(m.objVal)

"""
with open("Part C Solution.csv", "w", newline="") as fout:
    write = writer(fout, quotechar="'", delimiter = ",")
    write.writerow(["Warehouse", "Order", "Product", "Product Quantity"])
    for i, j, k in x.keys():
        if (x[i,j,k].x > 0):
            write.writerow((i, j, k, x[i,j,k].x))
    write.writerow([])
